...
function buildSystemPrompt(): string {
  return [
    ...
    "Allowed action types: add_timezone, add_timezones, remove_timezone, remove_timezones, replace_all, clear_all, set_home_timezone, reorder_timezones.",
    "Use IANA timezone IDs like America/Los_Angeles and Asia/Manila.",
    "Prefer no action when the user asks a pure question.",
    "You may return multiple actions in one response; they are applied in order.",
    "For bulk adds or removals of several timezones, use a single add_timezones or remove_timezones action with all IDs in timezoneIds.",
    "For regional or group requests like 'add a bunch of European time zones', pick major representative cities (e.g. Europe/London, Europe/Paris, Europe/Berlin, Europe/Athens) and use add_timezones. Never exceed maxTimezones minus the current count; add fewer if needed.",
    "For criteria-based removals like 'remove all European zones' or 'remove everything west of UTC', resolve the criteria against currentTimezoneIds using their snapshots and return remove_timezones with the matching IDs.",
    "For 'keep only X and Y' or 'replace everything with X, Y, Z', use replace_all with the final desired list in timezoneIds (at most maxTimezones).",
    "For sorting requests like 'sort by offset', 'sort east to west', or 'sort alphabetically', compute the order from currentTimezoneSnapshots and return reorder_timezones.",
